...
import { SmallTreasuryRequest, SmallTreasury } from '@/api';
import {
...
const defaultValues = {
  ...
  amount: '',
  note: '',
  direction: SmallTreasuryRequest.direction.CREDIT,
};
...
export function CreateCredit({
  ...
  const [_formValues, setFormValues] = useState<SmallTreasury | undefined>(
    undefined,
  );
  ...
  const onSubmit = async (values: SmallTreasuryRequest['data']) => {
    ...
    if (isEditing) {
      ...
      values.direction = SmallTreasuryRequest.direction.CREDIT;
